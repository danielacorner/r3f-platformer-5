...
export function EndermanModel({ scale = 1 }) {
  const { scene, animations } = useGLTF("/models/minecraft_better_enderman/scene.gltf");
  const group = useRef<Group>(null);
  const { actions } = useAnimations(animations, group);

  useEffect(() => {
    // Log available animations for debugging
    console.log("Available Enderman animations:", Object.keys(actions));
...
useGLTF.preload("/models/minecraft_better_enderman/scene.gltf");
