...
export function TowerManager({ towers }: TowerManagerProps) {
  ...
  const activeTowers = useRef<
    Map<string, { props: TowerProps; instanceId: number }>
  >(new Map());
  ...
  useEffect(() => {
    ...
    geometry.setAttribute(
      "instanceLevel",
      new THREE.BufferAttribute(levelArray, 1)
    );
    geometry.setAttribute(
      "instanceType",
    ...
    geometry.setAttribute(
      "instanceActive",
      new THREE.BufferAttribute(activeArray, 1)
    );
  ...
  useFrame((state, delta) => {
    ...
    activeTowers.current.forEach(({ props, instanceId }) => {
      ...
      tempObject.position.set(
        ...(position instanceof Vector3 ? position.toArray() : position)
      );
      if (target) {
  ...
  useEffect(() => {
    ...
    towers.forEach((towerProps) => {
      const key =
        towerProps.position instanceof Vector3
          ? towerProps.position.toArray().join(",")
          : towerProps.position.join(",");
    ...
    activeTowers.current.forEach(({ props }, key) => {
      if (
        !towers.find(
          (t) =>
            (t.position instanceof Vector3
              ? t.position.toArray().join(",")
              : t.position.join(",")) === key
        )
      ) {
...
export function Tower({
  ...
  const [lightnings, setLightnings] = useState<
    { start: Vector3; end: Vector3; id: number }[]
  >([]);
        ...
        prev
          .map((projectile) => ({
            ...
            position: projectile.position
              .clone()
              .add(projectile.velocity.clone().multiplyScalar(delta)),
  ...
  return (
    ...
    >
      ...
      >
        <cylinderGeometry
          args={[scaledWidth * 1.35, scaledWidth * 1.35, scaledHeight, 8]}
        />
      ...
      <group scale={preview ? previewScale : 1}>
        <mesh position={[0, 0.1, 0]} castShadow receiveShadow>
          <cylinderGeometry
            args={[scaledWidth * 0.7, scaledWidth * 0.8, 0.2, 8]}
          />
        ...
        {elementType === "light" && (
          <>
            ...
            <mesh position={[0, scaledHeight / 2 + 0.2, 0]} castShadow>
              <cylinderGeometry
                args={[0.2, scaledWidth * 0.5, scaledHeight, 6]}
              />
              <meshStandardMaterial
                color={stats.color}
            ...
            {[...Array(level)].map((_, i) => (
              <group key={i} rotation={[0, (Math.PI * 2 * i) / level, 0]}>
                <mesh
                  position={[0.4, scaledHeight * 0.7 + i * 0.2, 0]}
                  castShadow
                >
        ...
        {elementType === "fire" && (
          <>
            ...
            <mesh position={[0, scaledHeight / 2 + 0.2, 0]} castShadow>
              <cylinderGeometry
                args={[scaledWidth * 0.3, scaledWidth * 0.6, scaledHeight, 4]}
              />
              <meshStandardMaterial
                color="#8B0000"
                emissive={stats.emissive}
        ...
        {elementType === "ice" && (
          <>
            ...
            <mesh position={[0, scaledHeight / 2 + 0.2, 0]} castShadow>
              <cylinderGeometry
                args={[scaledWidth * 0.4, scaledWidth * 0.5, scaledHeight, 6]}
              />
              <meshStandardMaterial
                color={stats.color}
                emissive={stats.emissive}
            ...
            {[...Array(level + 2)].map((_, i) => (
              <group
                key={i}
                rotation={[0, (Math.PI * 2 * i) / (level + 2), Math.PI * 0.1]}
              >
        ...
        {elementType === "nature" && (
          <>
            ...
            <mesh position={[0, scaledHeight / 2 + 0.2, 0]} castShadow>
              <cylinderGeometry
                args={[scaledWidth * 0.3, scaledWidth * 0.4, scaledHeight, 6]}
              />
              <meshStandardMaterial
                color="#4B3621"
                transparent={preview}
            ...
            {[...Array(level + 1)].map((_, i) => (
              <group key={i} rotation={[0, (Math.PI * 2 * i) / (level + 1), 0]}>
                <mesh
                  position={[0.25, scaledHeight * (0.4 + i * 0.2), 0]}
                  castShadow
                >
        ...
        {elementType === "water" && (
          <>
            ...
            <mesh position={[0, scaledHeight / 2 + 0.2, 0]} castShadow>
              <cylinderGeometry
                args={[scaledWidth * 0.3, scaledWidth * 0.4, scaledHeight, 8]}
              />
              <meshStandardMaterial
                color={stats.color}
                emissive={stats.emissive}
        ...
        {elementType === "dark" && (
          <>
            ...
            <mesh position={[0, scaledHeight / 2 + 0.2, 0]} castShadow>
              <cylinderGeometry
                args={[scaledWidth * 0.2, scaledWidth * 0.4, scaledHeight, 4]}
              />
              <meshStandardMaterial
                color="#1a1a1a"
                emissive={stats.emissive}
            ...
            {[...Array(level)].map((_, i) => (
              <group key={i} rotation={[0, (Math.PI * 2 * i) / level, 0]}>
                <mesh
                  position={[0.3, scaledHeight * (0.3 + i * 0.2), 0]}
                  castShadow
                >
        ...
        {(preview || phase === "prep") && (
          <group>
            ...
            <mesh position={[0, 0.01, 0]} rotation={[-Math.PI / 2, 0, 0]}>
              ...
              <meshBasicMaterial
                color={
                  preview ? (canAfford ? "#44ff88" : "#ff4444") : "#ffffff"
                }
                transparent
