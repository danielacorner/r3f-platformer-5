...
export const PrimarySkillButton = styled.div<{ color?: string; empty?: boolean; isActive?: boolean }>`
  width: 100%;
  height: 100%;
  border-radius: 50%;
  background: ${props => props.empty ? 'rgba(20, 20, 30, 0.4)' : props.isActive ? `${props.color}22` : 'rgba(20, 20, 30, 0.8)'};
  ...
  box-shadow: ${props => props.empty ?
    ...
    props.isActive ?
      ...
      `0 0 20px ${props.color || '#60a5fa'}40, inset 0 0 15px ${props.color || '#60a5fa'}20`
  };
  z-index: 2;
  transition: all 0.3s ease;
  animation: ${props => props.isActive ? css`${activeGlowAnimation} 2s infinite` : 'none'};

  svg {
    width: 60%;
    height: 60%;
    color: ${props => props.empty ? 'rgba(255, 255, 255, 0.2)' : props.isActive ? props.color : props.color || '#60a5fa'};
    filter: ${props => props.empty ? 'none' : props.isActive ? `drop-shadow(0 0 8px ${props.color})` : `drop-shadow(0 0 5px ${props.color || '#60a5fa'})`};
  ...
  &:hover {
    background: ${props => props.empty ? 'rgba(30, 30, 40, 0.5)' : props.isActive ? `${props.color}33` : 'rgba(30, 30, 40, 0.9)'};
    box-shadow: ${props => props.empty ?
    'inset 0 0 20px rgba(255, 255, 255, 0.08)' :
    props.isActive ?
      `0 0 30px ${props.color}cc, 0 0 40px ${props.color}60, inset 0 0 25px ${props.color}60` :
      `0 0 25px ${props.color || '#60a5fa'}60, inset 0 0 20px ${props.color || '#60a5fa'}30`
  };
    svg {
...
export const SkillSlot = styled.div<{ color?: string; isActive?: boolean; index: number; total: number; empty?: boolean }>`
  ...
  box-shadow: ${props => props.empty ?
    ...
    props.isActive ?
      ...
      `0 0 15px ${props.color || '#60a5fa'}40, inset 0 0 10px ${props.color || '#60a5fa'}20`
  };
  pointer-events: auto;
  ...
  svg {
    ...
    props.isActive ? `brightness(1.3) drop-shadow(0 0 8px ${props.color})` : 'drop-shadow(0 0 3px currentColor)'};
    opacity: ${props => props.empty ? 0.3 : props.isActive ? 1 : 0.9};
    transition: all 0.3s ease;
    transform: ${props => `rotate(-${8 + -140 * (props.index / props.total)}deg) ${props.isActive ? 'scale(1.1)' : 'scale(1)'}`};
  }

  &:hover {
    background: ${props => props.empty ? 'rgba(30, 30, 40, 0.5)' :
    props.isActive ? `${props.color}44` : 'rgba(30, 30, 40, 0.9)'};
    box-shadow: ${props => props.empty ?
    'inset 0 0 15px rgba(255, 255, 255, 0.08)' :
    props.isActive ?
      `0 0 25px ${props.color}cc, 0 0 35px ${props.color}60, inset 0 0 20px ${props.color}60` :
      `0 0 20px ${props.color || '#60a5fa'}60, inset 0 0 15px ${props.color || '#60a5fa'}30`
  };
    svg {
      opacity: ${props => props.empty ? 0.4 : 1};
      transform: ${props => `rotate(-${8 + -140 * (props.index / props.total)}deg) ${props.isActive ? 'scale(1.15)' : 'scale(1.1)'}`};
    }
  }
